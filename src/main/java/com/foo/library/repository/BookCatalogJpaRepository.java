...
public interface BookCatalogJpaRepository extends
	...
	@Query("select new BookCatalog(b, bb.isActive, bb.isAvailable) from BookCatalog b "
			+ "left join b.books as bb "
			+ "where b.id in :bookCatalogs")
	List<BookCatalog> getCatalogWithAvailability(@Param("bookCatalogs") List<Long> bookCatalogIds);
...
//			+ "join BookCatalog b on (r.bookCatalog.id = b.id) "
